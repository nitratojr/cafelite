...
class AuthController {
    ...
    async login(req, res) {
        ...
        try {
            ...
            const senhaCorreta = user && await bcryptjs_1.default.compare(password, user.password);
            if (!user || !senhaCorreta) {
                return res.status(401).json({ error: 'Credenciais inválidas' });
            }
            // Gera um token JWT com o ID e e-mail do usuário, válido por 24 horas
            const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email }, process.env.JWT_SECRET, // Chave secreta definida no .env
            { expiresIn: '24h' });
